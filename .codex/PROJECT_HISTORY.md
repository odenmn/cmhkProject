...
## 2026-08-28：V1 改造 P5-A 客户返现规则、计划与期次

- 按用户确认的规则配置学生 Slash 30GB 12个月每月返16港元、学生 Slash 50GB 12个月每月返20港元，均从实际激活日满一个月开始、每月一期、共12期。
- 新增 `customer_cashback_rule`、`customer_cashback_plan`、`customer_cashback_installment`，并为 `mobile_plan_order` 增加 `activated_at`；规则、套餐及金额在计划生成时保存快照，后续修改规则不会回写历史计划。
- 订单选定匹配规则的套餐后先生成待激活返现计划；补录实际激活时间后才按满一个月口径生成12期，其他套餐不会被返现模块阻塞。
- 既有订单没有可靠实际激活时间时，不使用 `status_updated_at` 或上台日期推测；计划可先存在，但不会提前生成返现期次。
- 管理端新增“客户返现”页面，并在订单和客户详情中展示返现计划；管理员可维护规则、补生成计划及人工确认期次，运营人员不可见返现或佣金敏感金额。
- 返现期次人工确认只记录内部事实，不代表返现易到账或提现；返现易文件预览、确认、匹配、到账/提现事实和异常任务留待P5后续实现。
- 真实库先执行V005，再执行允许待激活计划的V006；批量生成前备份到 `D:\download\cmhk_before_existing_cashback_plans_2026-08-28.sql`（368636字节）。
- 通过临时18084业务API扫描142条现有订单，按套餐ID及可唯一匹配的旧套餐快照生成108条待激活计划：30GB 49条、50GB 59条；返现期次0条。
- 其余订单中33条套餐不匹配；订单3匹配30GB套餐但缺少客户关联，保留为冲突，未自动创建客户或覆盖关系。二次执行幂等，无重复订单计划。
- 核验结果：返现计划的订单、客户、规则悬空关系均为0，重复订单计划0；临时18084服务已关闭。
- 验证：后端33项测试中31项通过、2项按配置跳过；管理端生产构建通过；H5仍沿用上一轮已通过结果，管理端只有既有大分块警告。

## 2026-08-28：V1 改造 P6 基础数据分析

- 将管理端首页升级为基础分析页，支持时间范围、渠道筛选、核心转化、运营异常、资源库存、收益概览和渠道转化表。
- 指标口径集中在后端：上台为 `onboard_date` 非空；激活为标准状态已激活/已完成或激活状态已激活；上台率和激活率均以筛选订单数为分母。
- 时间范围分别按客户、订单、任务、对账行、佣金和返现计划的创建时间统计；ICCID库存明确为当前时点快照。
- 渠道范围账号可读取分析接口，但服务端强制使用令牌中的渠道ID；跨渠道查询会拒绝。非管理员只返回佣金和返现数量，不返回金额。
- 首页指标可跳转客户、订单、任务、对账异常、ICCID、渠道结算和返现计划；订单、任务和ICCID明细页接收首页状态查询参数。
- 真实接口核验：全局客户234、订单142、上台132、激活90、返现计划108、返现总额23568港元；渠道4客户225、订单130；2026年8月订单141。
- 验证：后端35项测试中33项通过、2项跳过；管理端和H5类型检查及生产构建通过；临时18085后端已关闭。
- P6没有新增数据库表或字段，没有修改真实业务数据；返现易外部文件仍按用户要求暂缓。
