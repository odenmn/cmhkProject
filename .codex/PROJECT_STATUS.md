...
- 阶段：JOINCOM × CMHK 管理后台 MVP 已完成首版；真实客户备份已按“预览 -> 确认 -> 事务写库”完成首批模拟导入及关系核验
...
1. 在管理端增加客户备份模拟、确认、批次明细和异常列表页面
2. 实现虚拟 ICCID 替换真实 ICCID 的事务接口，并写入 REPLACED 状态及分配历史
3. 人工处理 4 条格式异常和 1 组重复 ICCID 的 6 条异常明细
4. 为 Spring 上下文测试补充隔离的数据源配置，避免依赖开发机私有数据库配置
