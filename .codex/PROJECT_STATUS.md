...
- 阶段：JOINCOM × CMHK V1 改造阶段 P5 进行中；P5-A“客户返现规则、计划与期次”已完成，返现易导入暂缓
...
1. 进入P5后续前确认返现易文件格式、唯一匹配字段、到账状态和提现状态口径
2. 人工核实现有待激活返现计划对应订单的实际激活时间；补录可靠时间后自动生成12期
3. 由运营人员按真实业务人工新建接龙、导入候选号码并指定各条龙的初始龙头
4. 在测试数据上走通“分配龙头 -> 订单待激活 -> 上台号码成为新龙头”完整业务流程
5. 在管理端增加客户备份模拟、确认、批次明细和异常列表页面
6. 人工处理 4 条格式异常和 1 组重复 ICCID 的 6 条异常明细
7. 为 Spring 上下文测试补充隔离的数据源配置，避免依赖开发机私有数据库配置
