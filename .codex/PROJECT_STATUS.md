...
- 阶段：JOINCOM × CMHK V1 改造阶段 P4“任务与异常闭环”已完成；等待用户确认后再进入 P5
...
1. 等待用户确认是否进入 V1 改造阶段 P5
2. 由运营人员按真实业务人工新建接龙、导入候选号码并指定各条龙的初始龙头
3. 在测试数据上走通“分配龙头 -> 订单待激活 -> 上台号码成为新龙头”完整业务流程
4. 在管理端增加客户备份模拟、确认、批次明细和异常列表页面
5. 人工处理 4 条格式异常和 1 组重复 ICCID 的 6 条异常明细
