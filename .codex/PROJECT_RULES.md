...
- Java 代码以可读性优先：一条语句独占一行；不要将多个语句压缩到同一行；方法、公开接口和关键业务分支应补充必要的中文注释。
- Spring Bean 优先使用 `private final` 字段配合构造器注入。只有一个构造器时 Spring 会自动注入，不需要写 `@Autowired`；禁止新增字段注入，便于测试并避免隐藏依赖。
...
- `frontend/` 继续坚持移动端 H5 优先；`admin-frontend/` 是独立桌面管理端，不套用移动端布局限制。
- 管理端采用 Vue 3 + Vite + TypeScript + Element Plus，最低设计宽度为 1280px。
- 管理端视觉基准：220px 深棕侧栏、`#C81F1F` 品牌红、浅灰页面背景、白色卡片、红底白字表头。
- 管理端接口统一位于 `/api/admin/**`，必须使用独立管理员令牌，不得复用客户身份令牌。

## 5.2 长期业务约束

- 身份证件、地址证明、通行证、护照及 UMALL 要求的正式身份文件不得上传或存储在本系统。
- UMALL 正式订单、审核、上台、激活和合约状态以 UMALL 数据为准；JOINCOM 只接收、匹配、关联、计算和展示。
- 甲方文件导入必须先预览、后确认；无法唯一匹配的数据不得自动覆盖订单。
- Agent 只能通过业务 API 写入结果，不得直接操作数据库。
- 所有金额字段必须使用 MySQL `DECIMAL` 与 Java `BigDecimal`，禁止 `float`、`double`。
- 返现或渠道结算的实际付款必须保留人工审核，不得未经确认自动打款。
- 规则计算结果必须保存快照，规则修改不得回写历史订单金额。
