...
const createForm = reactive({ chainCode: '', chainName: '', remark: '' })
...
async function createChain() {
  try {
    ...
    Object.assign(createForm, { chainCode: '', chainName: '', remark: '' })
    ElMessage.success('接龙已创建，请导入或新增候选号码后指定龙头')
    await load()
...
<template>
  <div class="page">
    ...
    <el-dialog v-model="createVisible" title="新建接龙" width="500"><el-form label-position="top"><el-form-item label="接龙编码"><el-input v-model="createForm.chainCode" /></el-form-item><el-form-item label="接龙名称"><el-input v-model="createForm.chainName" /></el-form-item><el-form-item label="备注"><el-input v-model="createForm.remark" type="textarea" /></el-form-item></el-form><template #footer><el-button @click="createVisible = false">取消</el-button><el-button type="primary" @click="createChain">创建</el-button></template></el-dialog>
...
<style scoped>
.resource-metrics{grid-template-columns:repeat(4,minmax(0,1fr))}.resource-metrics .metric-card{min-height:100px;padding:18px}.resource-metrics .metric-value{margin-top:10px;font-size:25px}.referral-table{min-width:1200px}@media(max-width:1280px){.resource-metrics{grid-template-columns:repeat(2,minmax(0,1fr))}}
</style>
